...
namespace tools
{
    ...
    void Parser::parse( const std::string& filename )
    {
        ...
        while ( std::getline( file, line ) )
        {
            ...
            bool is_structure_start = line.find( "(#classinformer)" ) != std::string::npos; //: Line Contains "(#classinformer)" Class/Structure Identifier
...

            {
                if ( is_structure_start )
                {
                    if ( tokens[ 0 ] == ";" )
                    {
                        ...
                        if ( is_struct || is_class ) //: Token [ 1 ] Contains "class"/"struct" Keyword
                        {
                            if ( tokens[ 2 ].find( ":" ) != std::string::npos ) //: Token [ 2 ] Contains ":" (struct/class name termination) Keyword
                            {
                                if ( tokens[2].find( "::" ) != std::string::npos ) //: Token [ 2 ] Contains "::" (namespace) Keyword
                                {
                                    std::string namespace_name = tokens[ 2 ].substr( 0, tokens[ 2 ].find( "::" ) );
                                    std::string struct_name = tokens[ 2 ].substr( tokens[ 2 ].find( "::" ) + 2, tokens[ 2 ].rfind( ":" ) - ( tokens[ 2 ].find( "::" ) + 2 ) );
                                    ...
                                    for (int i = 3; i < tokens.size(); i++ ) //: For Each Inheritance Token [ i ]
                                    {
                                        if ( tokens[ i ].find( "," ) != std::string::npos ) //: Inheritance Token Delimiter
                                        {
                                            if ( ( tokens[ i ].find( "::" ) != std::string::npos ) ) //: Inheritance Token [ i ] Contains "::" (namespace) Keyword
                                            {
                                                std::string inherited_namespace_name = tokens[ i ].substr( 0, tokens[ i ].find( "::" ) );
                                                std::string inherited_struct_name = tokens[ i ].substr( tokens[ i ].find( "::" ) + 2, tokens[ i ].find( "," ) - ( tokens[ i ].find( "::" ) + 2 ) );
                                            ...
                                            else //: Inheritance Token [ i ] Does Not Contain "::" (namespace) Keyword
                                            {
                                                std::string inherited_struct_name = tokens[ i ].substr(0, tokens[ i ].find( "," ));
                                                this->structs[ struct_name ].struct_inheritance.insert( std::make_pair( inherited_struct_name, Struct() ) );
                                            }
                                        }
                                        else if ( tokens[ i ].find( ";" ) != std::string::npos ) //: Inheritance Token [ i ] Contains ";" (inheritance termination) Keyword
                                        {
                                            if ( ( tokens[ i ].find( "::" ) != std::string::npos ) ) //: Inheritance Token [ i ] Contains "::" (namespace) Keyword
                                            {
                                                std::string inherited_namespace_name = tokens[ i ].substr( 0, tokens[ i ].find( "::" ) );
                                ...
                                else //: Token [ 2 ] Does Not Contain "::" (namespace) Keyword
                                {
    ...
    void Parser::printCollectedInfo()
    {
        ...
        for (const auto& pair : this->structs)
        {
            ...
            if (!struct_obj.struct_inheritance.empty())
            {
                ...
                {
                    std::cout << inheritance_pair.second.name_space << "::" << inheritance_pair.first << ", ";
                }
            }
            std::cout << "\n{" << std::endl;
            for (const auto& member_pair : struct_obj.members)
            {
                ...
                std::cout << "\t" << member_obj.visibility << " " << member_obj.type << " " << member_name << ";" << std::endl;
            }
    ...
    void Parser::printCollectedInfoAsJson()
    {
        std::cout << "{\n";
        ...
        for (const auto& pair : this->structs)
        {
            ...
            std::cout << "\t\t{\n";
            std::cout << "\t\t\t\"type\": \"" << struct_obj.type << "\",\n";
            std::cout << "\t\t\t\"name\": \"" << struct_name << "\",\n";
            if (!struct_obj.name_space.empty())
            {
                std::cout << "\t\t\t\"namespace\": \"" << struct_obj.name_space << "\",\n";
            }
            if (!struct_obj.struct_inheritance.empty())
            {
            ...
            for (const auto& member_pair : struct_obj.members)
            {
                ...
                std::cout << "\t\t\t\t{\n";
                std::cout << "\t\t\t\t\t\"visibility\": \"" << member_obj.visibility << "\",\n";
                std::cout << "\t\t\t\t\t\"type\": \"" << member_obj.type << "\",\n";
            ...
            for (const auto& method_pair : struct_obj.methods)
            {
                ...
                std::cout << "\t\t\t\t{\n";
                std::cout << "\t\t\t\t\t\"visibility\": \"" << method_obj.visibility << "\",\n";
                std::cout << "\t\t\t\t\t\"return_type\": \"" << method_obj.return_type << "\",\n";
                std::cout << "\t\t\t\t\t\"name\": \"" << method_name << "\",\n";
                std::cout << "\t\t\t\t\t\"parameters\": [\n";
                for (const auto& param_pair : method_obj.parameters)
                {
                    std::cout << "\t\t\t\t\t\t{\n";
                    std::cout << "\t\t\t\t\t\t\t\"type\": \"" << param_pair.second.type << "\",\n";
                    std::cout << "\t\t\t\t\t\t\t\"name\": \"" << param_pair.first << "\"\n";
                    std::cout << "\t\t\t\t\t\t},\n";
                }
                std::cout << "\t\t\t\t\t]\n";
                std::cout << "\t\t\t\t},\n";
    ...
    void Parser::saveCollectedInfo()
    {
        for (const auto& pair : this->structs)
        {
            ...
            header_file << struct_obj.type << " ";
            if (!struct_obj.name_space.empty())
            {
                header_file << struct_obj.name_space << "::";
            }
            ...
            if (!struct_obj.struct_inheritance.empty())
            {
                ...
                for (const auto& inheritance_pair : struct_obj.struct_inheritance)
                {
                    header_file << inheritance_pair.second.name_space << "::" << inheritance_pair.first << ", ";
                }
            }
            header_file << "\n{" << std::endl;
            for (const auto& member_pair : struct_obj.members)
            {
                ...
                header_file << "\t" << member_obj.visibility << " " << member_obj.type << " " << member_name << ";" << std::endl;
            }
            for (const auto& method_pair : struct_obj.methods)
            {
                const std::string& method_name = method_pair.first;
                ...
                header_file << "\t" << method_obj.visibility << " " << method_obj.return_type << " " << method_name << "(";
                for (const auto& param_pair : method_obj.parameters)
                {
                    header_file << param_pair.second.type << " " << param_pair.first << ", ";
                }
